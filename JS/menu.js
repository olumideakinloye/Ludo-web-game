function gameMode() {
  document.getElementById("pop-up").innerHTML = `
          <div class="players">
            <p class="headers">Select mode</p>
            <div class="strict">
              <label for="strict">Strict Mode  </label
              ><input class="radio-number" type="radio" name="mode" value="strict" id="strict" />
            </div>
            <div class="easy">
              <label for="strict">Easy Mode  </label
              ><input class="radio-number" type="radio" name="mode" value="easy" id="easy" checked />
            </div>
            <div class="controles control-buttons">
              <button class="back-btn" onclick="playerNames()"><i class="fa-solid fa-turn-up arrow-icon-2" style=" transform: rotateZ(-90deg);"></i></button>
              <button onclick='moduleConnections("setGameMode", document.querySelector(".players input[name=${"mode"}]:checked").value)'>Next</button>
            </div>
          </div>`;
}
    ...
    `<div class='players-info players'>
          <div class='single-player-info player-1'>
            <input type='text' minlength='1' maxlength='23' placeholder='Enter your name' name='player-1' id='single-player-name'
              ...
              <button onclick='validatePlayerName()'>Next</button>
              </div>
          </div>
        </div>`;
}
...
function declearWinner(winner) {
  ...
  if (
    ...
    if (players.length == 2) {
      ...
      const playerInfos = document
        .getElementById("pop-up")
        .querySelectorAll(".player-info");
      if (playerInfos[0].clientWidth > playerInfos[1].clientWidth) {
        playerInfos[1].clientWidth = playerInfos[0].clientWidth;
      } else if (playerInfos[0].clientWidth < playerInfos[1].clientWidth) {
        playerInfos[0].clientWidth = playerInfos[1].clientWidth;
      }
    }
  }
