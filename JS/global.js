function validatePlayerName() {
  const input = document.getElementById("single-player-name");
  const value = input.value.trim();
  if(value.length > 0) {moduleConnections("setPlayerName", value)}else{return};
}
...
function offlineVersion(version, strict) {
  gameInfo["offline-mode"] = version;
  gameInfo["strict-mode"] = strict;
  if (version === "PlayPass") {
    NumberOfPlayers();
  } else if (version === "Bot") {
    NumberOfPlayers();
      ...
      for (let i = 0; i < colorToPlayerNumber.length; i++) {
        ...
        if (element.color == playerColor) {
          ...
          if (
            ...
            if (element.number == 3) {
              document.querySelector(`.player-2-info p`).innerText =
                `Bot${botNumber}`;
            } else if (element.number == 2) {
              document.querySelector(`.player-3-info p`).innerText =
                `Bot${botNumber}`;
            } else {
              document.querySelector(
                `.player-${element.number}-info p`,
